...
const CallsReport = () => {
  ...
  const fetchCalls = async () => {
    ...
    try {
      ...
      const response = await AnalyticsService.getCallsReport(params);
  ...
  const viewCall = async (call) => {
    ...
    try {
      // Fetch call details
      const detailsResponse = await AnalyticsService.getCallDetails(call.id);
      setModalData(detailsResponse.data.data);
      
      // Fetch transcript - use correct API method
      try {
        const transcriptResponse = await AnalyticsService.getCallTranscription(call.id);
        console.log('Transcript response:', transcriptResponse.data);
        setTranscript(transcriptResponse.data.data.messages || []);
      } catch (transcriptErr) {
        console.error('Failed to fetch transcript:', transcriptErr);
        // Don't fail the whole modal if transcript fails
        setTranscript([]);
      }
    } catch (err) {
  ...
  const exportCSV = async () => {
    try {
      ...
      const response = await AnalyticsService.exportCallsReport(params, 'csv');
      
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `calls_report_${Date.now()}.csv`);
  ...
  const StatusBadge = ({ status }) => {
    ...
    return (
      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${styles[status] || styles.in_progress}`}>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="mb-4">
          ...
          <h1 className="text-3xl font-bold text-gray-900">📞 Voice Calls Report</h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-7 gap-3 items-end">
          ...
          <div className="md:col-span-2">
            ...
            <input
              type="text"
              placeholder="Phone number, session ID..."
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div className="bg-white rounded-lg shadow p-6 text-center">
          <div className="text-xs text-gray-500 uppercase mb-2">Total Calls</div>
          <div className="text-2xl font-bold text-gray-900">{stats.total_calls?.toLocaleString() || 0}</div>
        </div>
        <div className="bg-white rounded-lg shadow p-6 text-center">
          <div className="text-xs text-gray-500 uppercase mb-2">Avg Duration</div>
          <div className="text-2xl font-bold text-gray-900">
            {AnalyticsService.formatDuration(stats.avg_duration_seconds || 0)}
          </div>
        </div>
        <div className="bg-white rounded-lg shadow p-6 text-center">
          <div className="text-xs text-gray-500 uppercase mb-2">Total Minutes</div>
          <div className="text-2xl font-bold text-gray-900">{stats.total_minutes?.toLocaleString() || 0}</div>
        </div>
        <div className="bg-white rounded-lg shadow p-6 text-center">
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">
            Call Records ({stats.total_calls?.toLocaleString() || 0} total)
          </h2>
        </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center h-64">
            <div className="text-center">
              <Phone className="w-12 h-12 text-primary-600 mx-auto mb-4 animate-pulse" />
                  ...
                  {calls.map((call) => (
                    ...
                    >
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {AnalyticsService.formatDate(call.start_time, 'datetime')}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {AnalyticsService.formatDuration(call.duration_seconds || 0)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <StatusBadge status={call.status} />
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <SentimentBadge sentiment={call.overall_sentiment} />
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {call.primary_intent || 'N/A'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {AnalyticsService.formatCurrency(call.final_cost)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <button 
                          onClick={(e) => { e.stopPropagation(); viewCall(call); }}
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-8 py-6 border-b border-gray-200 flex items-center justify-between sticky top-0 bg-white z-10">
              <h2 className="text-2xl font-semibold text-gray-900">
                📞 Call Details - Session #{selectedCall?.session_id?.substring(0, 12)}...
              </h2>
              <button
                ...
                <>
                  ...
                  <div className="grid grid-cols-3 gap-5 mb-8">
                    <div className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <div className="text-base font-semibold text-gray-900">
                        {AnalyticsService.formatDate(selectedCall?.start_time, 'datetime')}
                      </div>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <div className="text-xs text-gray-500 uppercase mb-1">Phone Number</div>
                      <div className="text-base font-semibold text-gray-900">
                        {selectedCall?.caller_id || 'Unknown'}
                      </div>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <div className="text-base font-semibold text-gray-900">
                        {AnalyticsService.formatDuration(selectedCall?.duration_seconds || 0)} min
                      </div>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <div className="text-base font-semibold text-gray-900">
                        {AnalyticsService.formatCurrency(selectedCall?.final_cost)}
                      </div>
                    </div>
                  </div>
                  ...
                  {modalData?.analytics && (
                    <div className="bg-gray-50 p-6 rounded-lg mb-8">
                      <h3 className="text-base font-semibold text-gray-900 mb-4">📊 Call Analytics</h3>
                      <div className="grid grid-cols-4 gap-4">
                        ...
                        <div className="bg-white p-4 rounded-lg text-center">
                          ...
                          <div className="text-xl font-bold text-gray-900">
                            {modalData.analytics.total_exchanges || transcript.length}
                          </div>
                        </div>
                        <div className="bg-white p-4 rounded-lg text-center">
                  ...
                  {modalData?.cost_breakdown && (
                    <div className="bg-gray-50 p-6 rounded-lg mb-8">
                      ...
                      <div className="grid grid-cols-4 gap-4">
                        ...
                        <div className="bg-white p-4 rounded-lg text-center">
                          <div className="text-xs text-gray-500 uppercase mb-1">Transcription</div>
                          <div className="text-xl font-bold text-gray-900">
                            ${modalData.cost_breakdown.transcription}
                          </div>
                        </div>
                        <div className="bg-white p-4 rounded-lg text-center">
                  ...
                  {/* Transcript */}
                  <div>
                    <h3 className="text-base font-semibold text-gray-900 mb-4">💬 Full Transcript</h3>
                    <div className="space-y-5">
                      {transcript.length === 0 ? (
                        <div className="bg-gray-50 p-8 rounded-lg text-center">
                          <p className="text-gray-500">No transcript available for this call</p>
                          <p className="text-sm text-gray-400 mt-2">Transcription may not have been generated yet</p>
                        </div>
                      ) : (
                        transcript.map((msg, idx) => (
                          <div
                            ...
                            className={`flex gap-4 p-4 bg-gray-50 rounded-lg border-l-4 ${
                              msg.speaker === 'agent' ? 'border-green-500' : 'border-red-500'
                            }`}
                          >
                            <div className="min-w-[120px]">
                              ...
                              <div className="text-xs text-gray-400">
                                {msg.timestamp ? new Date(msg.timestamp).toLocaleTimeString() : 
                                 msg.created_at ? AnalyticsService.formatDate(msg.created_at, 'time') : 'N/A'}
                              </div>
                            </div>
                            <div className="flex-1">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {msg.sentiment && (
                                  <span className="px-2 py-1 text-xs font-semibold bg-white border border-green-200 text-green-700 rounded-full">
