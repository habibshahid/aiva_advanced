...
=========================\n`;
    ...
    async _processBufferedInput(session, agent, bufferedData) {
		...
		if (bufferedData.images && bufferedData.images.length > 0) {
			...
			if (imageAnalysisHints) {
				// Build order_numbers array from extracted values
				const orderNumbers = [];
				
				// Add extracted_order_number if present
				if (imageAnalysisHints.extracted_order_number) {
					orderNumbers.push(imageAnalysisHints.extracted_order_number);
				}
				
				// Add extracted_phone if it could be used for lookup
				if (imageAnalysisHints.extracted_phone) {
					orderNumbers.push(imageAnalysisHints.extracted_phone);
				}
				
				// Create normalized extracted object
				imageAnalysisHints.extracted = {
					order_numbers: orderNumbers.length > 0 ? orderNumbers : undefined,
					phone: imageAnalysisHints.extracted_phone || undefined,
					email: imageAnalysisHints.extracted_email || undefined,
					tracking: imageAnalysisHints.extracted_tracking || undefined
				};
    ...
    async _handleDirectFunctionExecution(session, agent, llmDecision, bufferedData, context) {
        ...
        try {
            // Execute function directly - pass bufferedData for image access
            console.log(`⚡ Executing ${name} directly with params:`, params);
            const result = await this.executor.functionHandlers[name](params, agent, session, bufferedData);
    ...
    async _generateDirectFunctionResponse(session, agent, functionName, result, llmDecision, bufferedData, context) {
        ...
        // Format result for LLM based on function type
        let formattedResult = '';
        
        if (functionName === 'search_products' && result.products?.length > 0) {
            // Format products like ChatService does
            formattedResult = `=== MATCHING PRODUCTS ===\nFound ${result.products.length} products matching the image:\n\n`;
            
            result.products.slice(0, 5).forEach((product, index) => {
                formattedResult += `${index + 1}. **${product.name || product.title}**\n`;
                
                if (product.similarity_score || product.match_reason) {
                    const matchPct = product.similarity_score ? Math.round(product.similarity_score * 100) : null;
                    formattedResult += `   🎯 Match: ${product.match_reason || (matchPct + '% similarity')}\n`;
                }
                
                if (product.price) {
                    formattedResult += `   💰 Price: $${product.price}\n`;
                }
                
                if (product.metadata?.sku) {
                    formattedResult += `   🏷️ SKU: ${product.metadata.sku}\n`;
                }
                
                if (product.availability) {
                    formattedResult += `   📦 Stock: ${product.availability === 'in_stock' ? 'Available' : 'Out of Stock'}\n`;
                }
                
                if (product.metadata?.product_type) {
                    formattedResult += `   📂 Category: ${product.metadata.product_type}\n`;
                }
                
                if (product.url) {
                    formattedResult += `   🔗 URL: ${product.url}\n`;
                }
                
                formattedResult += '\n';
            });
            
            formattedResult += `=== END OF PRODUCTS (${result.products.length} total) ===`;
        } else if (functionName === 'search_products' && (!result.products || result.products.length === 0)) {
            formattedResult = '=== NO MATCHING PRODUCTS ===\nNo products found matching the search criteria.';
        } else {
            // Default: JSON stringify for other functions
...
Generate a helpful, natural response in the customer's language. Present the products in an attractive way with their key details (name, price, availability, link).`;
