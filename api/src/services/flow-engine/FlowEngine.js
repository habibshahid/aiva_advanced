...
class FlowEngine {
    ...
    async _processBufferedInput(session, agent, bufferedData) {
        ...
        // Add image analysis hints to context if available
        if (imageAnalysisHints) {
            context.image_analysis_hints = imageAnalysisHints;
        }
    ...
    _buildSystemPrompt(context) {
        ...
        // Add image analysis hints if available
        let imageHints = '';
        if (context.image_analysis_hints) {
            const hints = context.image_analysis_hints;
            imageHints = '\n\nIMAGE PRE-ANALYSIS (use this information):\n';
            imageHints += '- Image type: ' + (hints.image_type || 'unknown') + '\n';
            if (hints.extracted_order_number) {
                imageHints += '- EXTRACTED ORDER NUMBER: ' + hints.extracted_order_number + ' (USE THIS for order lookup)\n';
            }
            if (hints.extracted_phone) {
                imageHints += '- Extracted phone: ' + hints.extracted_phone + '\n';
            }
            if (hints.extracted_tracking) {
                imageHints += '- Extracted tracking: ' + hints.extracted_tracking + '\n';
            }
            if (hints.visible_damage) {
                imageHints += '- VISIBLE DAMAGE DETECTED: ' + (hints.damage_description || 'yes') + '\n';
            }
            imageHints += '- Suggested intent: ' + (hints.suggested_intent || 'general') + '\n';
            imageHints += '- Summary: ' + (hints.summary || 'Image provided') + '\n';
            imageHints += '\nIMPORTANT: If order number was extracted from image, use it immediately with check_order_status or start the appropriate flow with this data pre-filled.\n';
        }

        return context.instructions + '\n\n' +
'You are a conversational AI that routes customer messages to the appropriate flow.\n\n' +
'AVAILABLE FLOWS:\n' + flowsDescription + '\n' +
activeFlowInfo + pausedFlowsInfo + memoryInfo + customerInfo + kbInfo + imageHints + '\n\n' +
        ...
        switch (action) {
            case this.constructor.ACTIONS.START_FLOW:
                if (decision.flow_id) {
                    ...
                    // If flow is awaiting input and we have images, process them immediately
                    if (flowResult.status === 'awaiting_input' && bufferedData.images && bufferedData.images.length > 0) {
                        console.log(`🖼️ Flow awaiting input but we have ${bufferedData.images.length} images - processing immediately`);
                        
                        var flowResultWithImages = await this.executor.processFlowInput(
                            session.id,
                            {
                                message: bufferedData.combinedMessage,
                                images: bufferedData.images,
                        ...
                        result.flow = flowResultWithImages;
                        
                        if (flowResultWithImages.needs_llm_response && flowResultWithImages.llm_context) {
                            const llmResponse = await this._generateFunctionResponse(
                                flowResultWithImages.llm_context,
                                session,
                                agent,
                                bufferedData
                            );
                        ...
                        } else if (flowResultWithImages.message_to_send) {
                            result.response.text = flowResultWithImages.message_to_send;
                            result.response.html = this._formatHtml(result.response.text);
                            if (flowResultWithImages.status === 'flow_completed' && flowResultWithImages.completion_message) {
                                result.response.text += '\n\n' + flowResultWithImages.completion_message;
                                result.response.html = this._formatHtml(result.response.text);
                            }
                        } else if (flowResultWithImages.status === 'flow_completed' && flowResultWithImages.completion_message) {
                            result.response.text = flowResultWithImages.completion_message;
                            result.response.html = this._formatHtml(result.response.text);
                        } else if (flowResultWithImages.status === 'awaiting_input' && flowResultWithImages.prompt) {
                            result.response.text = flowResultWithImages.prompt;
                            result.response.html = this._formatHtml(result.response.text);
                        }
                    }
                    // Handle auto-executed function results
                    else if (flowResult.needs_llm_response && flowResult.llm_context) {
                        const llmResponse = await this._generateFunctionResponse(
            ...
            case this.constructor.ACTIONS.SWITCH_FLOW:
                ...
                if (decision.flow_id) {
                    ...
                    // If flow is awaiting input and we have images, process them immediately
                    if (flowResult2.status === 'awaiting_input' && bufferedData.images && bufferedData.images.length > 0) {
                        console.log(`🖼️ Flow awaiting input but we have ${bufferedData.images.length} images - processing immediately`);
                        
                        var flowResult2WithImages = await this.executor.processFlowInput(
                            session.id,
                            {
                                message: bufferedData.combinedMessage,
                                images: bufferedData.images,
                        ...
                        result.flow = flowResult2WithImages;
                        
                        if (flowResult2WithImages.needs_llm_response && flowResult2WithImages.llm_context) {
                            const llmResponse = await this._generateFunctionResponse(
                                flowResult2WithImages.llm_context,
                                session,
                                agent,
                                bufferedData
                            );
                        ...
                        } else if (flowResult2WithImages.message_to_send) {
                            result.response.text = flowResult2WithImages.message_to_send;
                            result.response.html = this._formatHtml(result.response.text);
                            if (flowResult2WithImages.status === 'flow_completed' && flowResult2WithImages.completion_message) {
                                result.response.text += '\n\n' + flowResult2WithImages.completion_message;
                                result.response.html = this._formatHtml(result.response.text);
                            }
                        } else if (flowResult2WithImages.status === 'flow_completed' && flowResult2WithImages.completion_message) {
                            result.response.text = flowResult2WithImages.completion_message;
                            result.response.html = this._formatHtml(result.response.text);
                        } else if (flowResult2WithImages.status === 'awaiting_input' && flowResult2WithImages.prompt) {
                            result.response.text = flowResult2WithImages.prompt;
                            result.response.html = this._formatHtml(result.response.text);
                        }
                    }
                    // Handle auto-executed function results
                    else if (flowResult2.needs_llm_response && flowResult2.llm_context) {
                        const llmResponse = await this._generateFunctionResponse(
            ...
            case this.constructor.ACTIONS.CONTINUE_FLOW:
                ...
                if (wantsDifferentFlow) {
                    ...
                    // If flow is awaiting input and we have images, process them immediately
                    if (flowResultSwitch.status === 'awaiting_input' && bufferedData.images && bufferedData.images.length > 0) {
                        console.log(`🖼️ Flow awaiting input but we have ${bufferedData.images.length} images - processing immediately`);
                        
                        var flowResultSwitchWithImages = await this.executor.processFlowInput(
                            session.id,
                            {
                                message: bufferedData.combinedMessage,
                                images: bufferedData.images,
                        ...
                        result.flow = flowResultSwitchWithImages;
                        
                        if (flowResultSwitchWithImages.needs_llm_response && flowResultSwitchWithImages.llm_context) {
                            const llmResponse = await this._generateFunctionResponse(
                                flowResultSwitchWithImages.llm_context,
                                session,
                                agent,
                                bufferedData
                            );
                        ...
                        } else if (flowResultSwitchWithImages.message_to_send) {
                            result.response.text = flowResultSwitchWithImages.message_to_send;
                            result.response.html = this._formatHtml(result.response.text);
                            if (flowResultSwitchWithImages.status === 'flow_completed' && flowResultSwitchWithImages.completion_message) {
                                result.response.text += '\n\n' + flowResultSwitchWithImages.completion_message;
                                result.response.html = this._formatHtml(result.response.text);
                            }
                        } else if (flowResultSwitchWithImages.status === 'flow_completed' && flowResultSwitchWithImages.completion_message) {
                            result.response.text = flowResultSwitchWithImages.completion_message;
                            result.response.html = this._formatHtml(result.response.text);
                        } else if (flowResultSwitchWithImages.status === 'awaiting_input' && flowResultSwitchWithImages.prompt) {
                            result.response.text = flowResultSwitchWithImages.prompt;
                            result.response.html = this._formatHtml(result.response.text);
                        }
                    }
                    // Handle flow result normally
                    else if (flowResultSwitch.needs_llm_response && flowResultSwitch.llm_context) {
                        const llmResponse = await this._generateFunctionResponse(
                    ...
                    // If flow is awaiting input and we have images, process them immediately
                    // This handles the case where user sends image and flow starts with image collect step
                    if (flowResult3b.status === 'awaiting_input' && bufferedData.images && bufferedData.images.length > 0) {
                        console.log(`🖼️ Flow awaiting input but we have ${bufferedData.images.length} images - processing immediately`);
                        
                        // Process the images through the flow
                        var flowResultWithImages = await this.executor.processFlowInput(
                            session.id,
                            {
                                message: bufferedData.combinedMessage,
                        ...
                        // Handle the result from processing images
                        if (flowResultWithImages.needs_llm_response && flowResultWithImages.llm_context) {
                            const llmResponse = await this._generateFunctionResponse(
                                flowResultWithImages.llm_context,
                                session,
                    ...
                    // Handle flow result normally (no images to process)
                    else if (flowResult3b.needs_llm_response && flowResult3b.llm_context) {
                        const llmResponse = await this._generateFunctionResponse(
...
This is required to continue the flow. Do not skip this.\n`;
    ...
    /**
     * Pre-analyze images before LLM decision when no active flow
     * Extracts order numbers, detects image type, provides hints for routing
     */
    async _preAnalyzeImages(images, agent) {
        if (!images || images.length === 0) return null;
        
        try {
            const LLMService = require('../LLMService');
            
            const systemPrompt = `You are an image analyzer for a customer service system. Analyze the image(s) and extract useful information.

DETECT AND EXTRACT:
1. ORDER INFORMATION:
   - Order numbers (formats: CZ-XXXXX, XXXXX, #XXXXX)
   - Tracking numbers
   - Phone numbers (Pakistani format: 03XX or +92)
   - Email addresses

2. IMAGE TYPE:
   - order_screenshot: Screenshot of an order confirmation, tracking page, or receipt
   - damage_photo: Photo showing damaged/broken product
   - product_photo: Photo of a product (for identification or search)
   - size_label: Photo showing size tag or label
   - receipt_photo: Physical receipt or invoice
   - other: Anything else

3. VISIBLE ISSUES (if any):
   - Damage type (torn, broken, scratched, etc.)
   - Quality issues

Respond in JSON format:
{
  "image_type": "order_screenshot|damage_photo|product_photo|size_label|receipt_photo|other",
  "extracted_order_number": "CZ-XXXXX or null if not found",
  "extracted_phone": "phone number or null",
  "extracted_tracking": "tracking number or null",
  "visible_damage": true/false,
  "damage_description": "description or null",
  "confidence": "high|medium|low",
  "suggested_intent": "check_order|report_damage|product_inquiry|size_exchange|general",
  "summary": "Brief description of what the image shows"
}`;

            // Build messages with images
            const userContent = [
                { type: 'text', text: 'Analyze this image and extract any useful information:' }
            ];
            
            for (const image of images.slice(0, 2)) { // Limit to 2 images for cost
                userContent.push({
                    type: 'image_url',
            ...
            try {
                ...
                console.error('Failed to parse image analysis:', e.message);
                return null;
            }
            
            console.log('🖼️ Image pre-analysis cost:', completion.cost?.final_cost);
            ...
            console.error('Error in pre-image analysis:', error);
            return null;
        }
    }
